...
func (r *Repository) enrichMessage(ctx context.Context, msg *model.Message) error {
// 处理群聊消息
if msg.IsChatRoom {
// 补充群聊名称
...
if msg.IsSelf {
// 对于自己发送的消息，使用account作为UserName
senderUserName = r.account
} else {
senderUserName = msg.Sender
}

if senderUserName != "" {
contact := r.getFullContact(senderUserName)
if contact != nil {
msg.SenderAvatar = contact.SmallHeadImgUrl
msg.SenderAvatarHD = contact.BigHeadImgUrl
}
}
