...
func (m *Manager) GetCurrentStatus() *ctx.Context {
	// If context is not initialized, try to initialize it for status checking
	if m.ctx == nil {
		var err error
		m.ctx, err = ctx.New("")
		if err != nil {
			// If we can't create context, return a minimal one for status
			m.ctx = &ctx.Context{
				HTTPEnabled: false,
				HTTPAddr:    "",
			}
		...
		// Initialize minimal services for status checking
		if m.wechat == nil {
			m.wechat = wechat.NewService(m.ctx)
		}
	}
...
func (m *Manager) isHTTPServiceRunning() bool {
	// Get the HTTP address from context or server config
	var addr string
	if m.ctx != nil && m.ctx.HTTPAddr != "" {
		addr = m.ctx.HTTPAddr
	} else if m.sc != nil {
		addr = m.sc.GetHTTPAddr()
	}
